...
int wm8994_ampcontrol_playback(int on)
{
	...
	//DEBUG_LOG_ERR("++ ampctrl play %#x", on);
	if(on) {
		wm8994->stream_state |= (PCM_STREAM_PLAYBACK);
		wm8994->codec_state |= (PLAYBACK_ACTIVE);
...
int wm8994_ampcontrol_record(int on)
{
	...
	//DEBUG_LOG_ERR("++ ampctrl rec %#x", on);
	if(on) {
		wm8994->stream_state |= (PCM_STREAM_CAPTURE);
		wm8994->codec_state |= (CAPTURE_ACTIVE);
		audio_ctrl_mic_bias_gpio(codec, 'A');
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	DEBUG_LOG_ERR("set_rec_path param %d", path_num);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	int path_num = ucontrol->value.integer.value[0];
	DEBUG_LOG_ERR("set_playback_path param %d", path_num);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	DEBUG_LOG_ERR("set_voicecall_path param %d", path_num);

	switch(path_num)
	{
		case OFF:
			DEBUG_LOG_ERR("Switching off output path\n");
			break;
		...
		case HP_NO_MIC: // 3POLE
		case BT:
			DEBUG_LOG_ERR("routing VOICE PATH to %s", mc->texts[path_num] );
			break;
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	DEBUG_LOG_ERR("set_voipcall_path param %d", path_num);
	...
	switch(path_num)
	{
		...
		case HP_NO_MIC: // 3POLE
		case BT:
			DEBUG_LOG_ERR("routing voip path to %s", mc->texts[path_num] );
			break;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	case CMD_CALL_FLAG_CLEAR:
		DEBUG_LOG_ERR("Call Flag is clear!!");
		//wm8994->codec_state &= ~(CALL_ACTIVE);
		break;

	case CMD_CALL_END :
		DEBUG_LOG_ERR("Call shutdown function forcely for call!!");
		//wm8994->codec_state &= ~(CALL_ACTIVE);
	...
	case CMD_CODEC_STANDBY :
		DEBUG_LOG_ERR("Codec Standby is called!!");
		wm8994->codec_state &= ~(PLAYBACK_ACTIVE);
		wm8994->stream_state &= ~(PCM_STREAM_PLAYBACK);
		break;
	...
	case CMD_CODEC_EMERGENCY_RECOVERY:
		...
		wm8994_write(codec,0x210, 0x0073);
		wm8994_write(codec,0x300, 0x4010);
		DEBUG_LOG_ERR(" INITIALIZE ***** RECOVERY *****");
			  ...
			  struct snd_soc_dai *codec_dai)
{
	...
	if (wm8994->power_state == CODEC_OFF) {
		...
		wm8994_write(codec,0x210, 0x0073);
		wm8994_write(codec,0x300, 0x4010);
		DEBUG_LOG_ERR(" INITIALIZE ***** START UP *****");
	} else {
...
static int wm8994_resume(struct snd_soc_codec *codec)
{
	...
	if (wm8994->power_state == CODEC_OFF) {
		...
		wm8994_write(codec,0x210, 0x0073);
		wm8994_write(codec,0x300, 0x4010);
		DEBUG_LOG_ERR(" INITIALIZE ***** RESUME *****");
	}
	return 0;
}
#endif
